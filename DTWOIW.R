...
#Euclidean OIW
#DTW Full Window
Q_dtw <- function(v1,v2,w=0){
  oiwindow <- w
  v1 <- invariateWindow(v1,oiwindow)
  v2 <- invariateWindow(v2,oiwindow)
  alignment <- dtw(v1,v2 , step.pattern = symmetric1 , distance.only = TRUE )
  dres <- alignment$distance
  dres
}
...
train <- as.matrix(read.csv("" , header = FALSE ))
#Loading Test Dataset
test <- as.matrix(read.csv("", header = FALSE))
